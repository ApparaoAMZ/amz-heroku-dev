...
public class AnonymizationFileProcessorTest {
	...
	private List<AnonymizationInputView> anonymizationProcessorData() {
		...
		AnonymizationInputView anonymizationInputView2 = new AnonymizationInputView("Interview_c", "Candidate Email",
				...
				"Null/Empty/Privacy Deleted", "Privacy Deleted", "EUR-EU", "AUT");
		AnonymizationInputView anonymizationInputView3 = new AnonymizationInputView("Interview_c", "Candidate Email",
				"CANDIDATE_EMAIL__C", "Formula (Text)", "Pre-Hire: Candidate-provided data of unsuccessful candidates",
				"Null/Empty/Privacy Deleted", "Privacy Deleted", "EUR-EU", "AUT");
		anonymizationInputView3.getFieldLabel();
		anonymizationInputView3.getType() ;
		anonymizationInputView3.getCategoryName() ;
		anonymizationInputView3.getRecommendedTransformation() ;
		anonymizationInputView3.getChosenTransformation() ; 
		anonymizationInputView3.getRegion()  ;
		anonymizationInputView3.getCountryCode();
		anonymizationInputView3.equals(anonymizationInputView2);
		lstAnonymizationInputView.add(anonymizationInputView2);
		lstAnonymizationInputView.add(anonymizationInputView1);
		return lstAnonymizationInputView;
	}
	...
	@Test
	public void parseAnonymizationFileTest() throws GdprException, IOException {
		MultipartFile file = new MockMultipartFile("HerokuAnonymization-V0.1", "HerokuAnonymization-V0.1.xlsx", "application/vnd.ms-excel", 
				new ClassPathResource("HerokuAnonymization-V0.1.xlsx").getInputStream());
		int expected=186;
		List<AnonymizationInputView>  response=anonymizationProcessor.parseAnonymizationFile(file);
