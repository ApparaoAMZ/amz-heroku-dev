...
public class RunMgmtDaoImplTest {
	...
	private List<RunMgmt> lstrunMgmt() {
		/*
		 * setRunStartDateTime(Date) 1 setRunEndDateTime(Date) setRunComments(String)
		 * getRunName() getRunStartDateTime() getRunEndDateTime() getRunComments()
		 */
		List<RunMgmt> lstrunMgmt = new ArrayList<RunMgmt>();
		RunMgmt runMgmt1 = new RunMgmt();
		runMgmt1.setRunId(2L);
		runMgmt1.setRunName("Test");
		runMgmt1.setRunStatus("SUCCESS");

		RunMgmt runMgmt3 = new RunMgmt();
		long millis = System.currentTimeMillis();
		java.sql.Date date = new java.sql.Date(millis);
		runMgmt3.setRunStartDateTime(date);
		runMgmt3.setRunEndDateTime(date); 
		runMgmt3.setRunComments("Test");
		runMgmt3.setRunName("TestRun");
		runMgmt3.getRunName(); 
		runMgmt3.getRunStartDateTime();
		runMgmt3.getRunEndDateTime();
		runMgmt3.getRunComments();
		RunMgmt runMgmt4=new RunMgmt(7L, "Test", "Test", date, date, "Test"); 

		RunMgmt runMgmt2 = new RunMgmt(7L,"Test");
		runMgmt2.setRunName("Test");
		runMgmt2.getRunName();
		lstrunMgmt.add(runMgmt1);
		return lstrunMgmt;
	}
